...
show_help() {
  cat <<EOF
Usage:
  GPU=0 bash care_myocardium/scripts/train_scar_roi_refiner.sh <fold> [extra args]

Runs the second-stage CARE scar ROI refiner on Dataset603.
...
echo "[CARE scar ROI refiner] dataset=${CARE_DATASET_ID} trainer=${TRAINER} epochs=${SCAR_ROI_EPOCHS}"
echo "[CARE scar ROI refiner] ext_trainer=${nnUNet_extTrainer}"
