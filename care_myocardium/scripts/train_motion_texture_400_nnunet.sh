...
show_help() {
  cat <<EOF
Usage:
  GPU=0 bash care_myocardium/scripts/train_motion_texture_400_nnunet.sh <fold> [extra args]

Runs the paper-aligned 400-epoch motion-texture nnU-Net trainer.
...
Environment overrides:
  ...
  MOTION_TEXTURE_EPOCHS  default 400
...
export TRAINER="${TRAINER:-MotionTexture400EpochTrainer}"
export MOTION_TEXTURE_EPOCHS="${MOTION_TEXTURE_EPOCHS:-400}"
...
echo "[CARE motion-texture 400] dataset=${CARE_DATASET_ID} trainer=${TRAINER} epochs=${MOTION_TEXTURE_EPOCHS}"
echo "[CARE motion-texture 400] ext_trainer=${nnUNet_extTrainer}"
